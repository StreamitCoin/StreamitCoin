...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("streamitcoin-wallet");
...
bool CWalletDB::WriteZSTREAMSeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZSTREAMSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZSTREAMSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZSTREAMSeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZSTREAMSeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZSTREAMSeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZSTREAMCount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZSTREAMCount(uint32_t& nCount)
{
