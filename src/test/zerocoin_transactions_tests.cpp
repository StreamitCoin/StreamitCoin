...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zpivTracker = unique_ptr<CzSTREAMTracker>(new CzSTREAMTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZSTREAM_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZSTREAM_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
