...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzSTREAMWallet *czSTREAMWallet = new CzSTREAMWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czSTREAMWallet->GenerateDeterministicZSTREAM(denom, coin, dMint, true);
        czSTREAMWallet->UpdateCount();
