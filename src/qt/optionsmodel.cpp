...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::STREAM);
    ...
    if (!settings.contains("nAnonymizeStreamitcoinAmount"))
        settings.setValue("nAnonymizeStreamitcoinAmount", 1000);

    nAnonymizeStreamitcoinAmount = settings.value("nAnonymizeStreamitcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeStreamitcoinAmount"))
        SoftSetArg("-anonymizestreamitcoinamount", settings.value("nAnonymizeStreamitcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in streamitcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeStreamitcoinAmount:
            return QVariant(nAnonymizeStreamitcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeStreamitcoinAmount:
            nAnonymizeStreamitcoinAmount = value.toInt();
            settings.setValue("nAnonymizeStreamitcoinAmount", nAnonymizeStreamitcoinAmount);
            emit anonymizeStreamitcoinAmountChanged(nAnonymizeStreamitcoinAmount);
            break;
        case CoinControlFeatures:
