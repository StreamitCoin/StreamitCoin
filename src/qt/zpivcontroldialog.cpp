    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSTREAM.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSTREAM seed used to mint this zSTREAM is not the same as currently hold in the wallet";
            else
