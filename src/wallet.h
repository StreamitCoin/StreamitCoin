...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 STREAM at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSTREAM send
enum ZerocoinSpendStatus {
    ZSTREAM_SPEND_OKAY = 0,                            // No error
    ZSTREAM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSTREAM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSTREAM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSTREAM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSTREAM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSTREAM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSTREAM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSTREAM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSTREAM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSTREAM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSTREAM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSTREAM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSTREAM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSTREAM_SPENT_USED_ZSTREAM = 14,                      // Coin has already been spend
    ZSTREAM_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSTREAM_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSTREAMOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzSTREAMTracker> zpivTracker;
    ...
    void setZWallet(CzSTREAMWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzSTREAMTracker>(new CzSTREAMTracker(strWalletFile));
    }

    CzSTREAMWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSTREAM reset */
    boost::signals2::signal<void()> NotifyzSTREAMReset;
