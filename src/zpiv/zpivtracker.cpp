...
CzSTREAMTracker::CzSTREAMTracker(std::string strWalletFile)
{
...
CzSTREAMTracker::~CzSTREAMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSTREAMTracker::Init()
{
...
bool CzSTREAMTracker::Archive(CMintMeta& meta)
{
...
bool CzSTREAMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSTREAMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSTREAMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSTREAMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSTREAMTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSTREAMTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSTREAMTracker::GetSerialHashes()
{
...
CAmount CzSTREAMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSTREAMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSTREAMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSTREAMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSTREAMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSTREAMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSTREAMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSTREAMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSTREAMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSTREAMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSTREAMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSTREAMWallet* zSTREAMWallet)
{
    bool iszSTREAMWalletInitialized = (NULL != zSTREAMWallet);
    CMintMeta meta;
    ...
    if (! iszSTREAMWalletInitialized)
        zSTREAMWallet = new CzSTREAMWallet(strWalletFile);
    meta.isSeedCorrect = zSTREAMWallet->CheckSeed(dMint);
    if (! iszSTREAMWalletInitialized)
        delete zSTREAMWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSTREAMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSTREAMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSTREAMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSTREAMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSTREAMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSTREAMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSTREAMWallet* zSTREAMWallet = new CzSTREAMWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zSTREAMWallet);
        }
        delete zSTREAMWallet;
