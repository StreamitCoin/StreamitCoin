...
CzSTREAMWallet::CzSTREAMWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSTREAMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSTREAMSeed_deprecated()) {
                    LogPrintf("%s: Updated zSTREAM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzSTREAMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSTREAMCount(nCountLastUsed);
    else if (!walletdb.ReadZSTREAMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSTREAMWallet::Lock()
{
    seedMaster = 0;
}

void CzSTREAMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSTREAMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSTREAM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSTREAMWallet::LoadMintPoolFromDB()
{
...
void CzSTREAMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSTREAMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSTREAMWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSTREAMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSTREAM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzSTREAMWallet::SeedToZSTREAM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSTREAMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSTREAMWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZSTREAMCount(nCountLastUsed);
...
void CzSTREAMWallet::GenerateDeterministicZSTREAM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSTREAMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSTREAMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSTREAMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
