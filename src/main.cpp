...
#if defined(NDEBUG)
#error "StreamitCoin cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zSTREAM spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zSTREAM is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zSTREAM spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zSTREAM without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zSTREAM spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSTREAM spend tx %s already in block %d",
            ...
            // Check that zSTREAM mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 9490000 * COIN;
    }
    else if (nHeight < 20000 && nHeight > 0) {
        nSubsidy = 3.5 * COIN;
    }
    else if (nHeight <= 39999 && nHeight >= 20000) {
        nSubsidy = 4 * COIN;
    }
    else if (nHeight <= 59999 && nHeight >= 40000) {
        nSubsidy = 4.5 * COIN;
    }
    else if (nHeight <= 79999 && nHeight >= 60000) {
        nSubsidy = 5 * COIN;
    }
    else if (nHeight <= 99999 && nHeight >= 80000) {
        nSubsidy = 5.5 * COIN;
    }
    else if (nHeight <= 119999 && nHeight >= 100000) {
        nSubsidy = 6 * COIN;
    }
    else if (nHeight <= 139999 && nHeight >= 120000) {
        nSubsidy = 6.5 * COIN;
    }
    else if (nHeight <= 159999 && nHeight >= 140000) {
        nSubsidy = 7 * COIN;
    }
    else if (nHeight <= 179999 && nHeight >= 160000) {
        nSubsidy = 7.5 * COIN;
    }
    else if (nHeight <= 199999 && nHeight >= 180000) {
        nSubsidy = 8 * COIN;
    }
    else if (nHeight <= 219999 && nHeight >= 200000) {
        nSubsidy = 8.5 * COIN;
    }
    else if (nHeight <= 239999 && nHeight >= 220000) {
        nSubsidy = 9 * COIN;
    }
    else if (nHeight <= 259999 && nHeight >= 240000) {
        nSubsidy = 9.5 * COIN;
    }
    else if (nHeight <= 279999 && nHeight >= 260000) {
        nSubsidy = 10 * COIN;
    }
    else if (nHeight <= 299999 && nHeight >= 280000) {
        nSubsidy = 9.8 * COIN;
    }
    else if (nHeight <= 319999 && nHeight >= 300000) {
        nSubsidy = 9.6 * COIN;
    }
    else if (nHeight <= 339999 && nHeight >= 320000) {
        nSubsidy = 9.4 * COIN;
    }
    else if (nHeight <= 359999 && nHeight >= 340000) {
        nSubsidy = 9.2 * COIN;
    }
    else if (nHeight <= 379999 && nHeight >= 360000) {
        nSubsidy = 9 * COIN;
    }
    else if (nHeight <= 399999 && nHeight >= 380000) {
        nSubsidy = 8.8 * COIN;
    }
    else if (nHeight <= 419999 && nHeight >= 400000) {
        nSubsidy = 8.6 * COIN;
    }
    else if (nHeight <= 439999 && nHeight >= 420000) {
        nSubsidy = 8.4 * COIN;
    }
    else if (nHeight <= 459999 && nHeight >= 440000) {
        nSubsidy = 8.2 * COIN;
    }
    else if (nHeight <= 479999 && nHeight >= 460000) {
        nSubsidy = 8 * COIN;
    }
    else if (nHeight <= 499999 && nHeight >= 480000) {
        nSubsidy = 7.8 * COIN;
    }
    else if (nHeight <= 519999 && nHeight >= 500000) {
        nSubsidy = 7.6 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSTREAMStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from StreamitCoin
...
{
    RenameThread("streamitcoin-scriptch");
...
void RecalculateZSTREAMMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZSTREAM..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZSTREAM..."), percent);
        }
...
void RecalculateZSTREAMSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZSTREAM..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZSTREAM..."), percent);
        }

        //Rewrite zSTREAM supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zSTREAM supply
...
bool RecalculateSTREAMSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating STREAM supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating STREAM supply..."), percent);
        }
...
{
    // StreamitCoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZSTREAMSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zSTREAM supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                         !((pindex->nHeight == 0 && pindex->GetBlockHash() == uint256("0000075f78e3eb5ffe4ce2eb3b708bfa86caa15fe87cf8eaa9971f064f5a1c2b")));
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZSTREAMMinted();
        RecalculateZSTREAMSpent();
        RecalculateSTREAMSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSTREAM money supply in the block index
    if (!UpdateZSTREAMSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zSTREAM supply for block=%s height=%d", __func__,
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert STREAM to zSTREAM
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSTREAM spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSTREAM serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSTREAM stake block is less than allowed start height", __func__);

    if (CZPivStake* zSTREAM = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zSTREAM->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSTREAM stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSTREAM stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSTREAM->GetValue()));
        if (nChecksum200 != zSTREAM->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSTREAM->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZSTREAM() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zSTREAM fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        std::vector<CTxIn> zSTREAMInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zSTREAMInputs.push_back(stakeIn);
            }else{
                pivInputs.push_back(stakeIn);
            }
        }
        const bool hasSTREAMInputs = !pivInputs.empty();
        const bool hasZSTREAMInputs = !zSTREAMInputs.empty();
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zSTREAM inputs.
            if(hasZSTREAMInputs){
                for (const CTxIn& zPivInput : zSTREAMInputs) {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSTREAM mints and %d zSTREAM spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // StreamitCoin: We use certain sporks during IBD, so check to see if they are
