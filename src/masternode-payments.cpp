...
#define REV_BLOCK 260000
#define PHZ_BLOCK 20000
    ...
    if (!masternodeSync.IsSynced()) { //there is no budget data to use to check anything
        ...
        if (nHeight % GetBudgetPaymentCycleBlocks() < 100) {
            ...
            if (nHeight <= REV_BLOCK && nHeight % PHZ_BLOCK != 0 && nMinted > nExpectedValue) {
                if (fDebug)
                    LogPrint("masternode","Reverse Block: Reward is not pays actual=%s > limit=%s Blocks=%s\n",
                ...
                if (fDebug)
                    LogPrint("masternode","Reverse Block: Reward pays actual=%s limit=%s Blocks=%s\n",
                            FormatMoney(nMinted), FormatMoney(nExpectedValue), nHeight);
            }
        }
...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZSTREAMStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZSTREAMStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZSTREAMStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZSTREAMStake);
        ...
        BOOST_FOREACH (CTxOut out, txNew.vout) {
            if (payee.scriptPubKey == out.scriptPubKey) {
                if(nBlockHeight <= REV_BLOCK && out.nValue <= requiredMasternodePayment) {
                    found = true;
                    if (fDebug)
                        LogPrint("masternode","Masternode payment was made successfully. Paid=%s <= Max=%s Reward=%s Blocks=%s\n",
                                  FormatMoney(out.nValue).c_str(), FormatMoney(requiredMasternodePayment).c_str(), FormatMoney(nReward).c_str(), nBlockHeight);
                }
                else if(nBlockHeight > REV_BLOCK && out.nValue >= requiredMasternodePayment) {
                    found = true;
                    if (fDebug)
                        LogPrint("masternode","Masternode payment was made successfully. Paid=%s => Min=%s Reward=%s Blocks=%s\n",
                                  FormatMoney(out.nValue).c_str(), FormatMoney(requiredMasternodePayment).c_str(), FormatMoney(nReward).c_str(), nBlockHeight);
                }
                else
                    LogPrint("masternode","Masternode payment is out of drift range. Paid=%s Min=%s Reward=%s Blocks=%s\n",
                              FormatMoney(out.nValue).c_str(), FormatMoney(requiredMasternodePayment).c_str(), FormatMoney(nReward).c_str(), nBlockHeight);
            }
        }
