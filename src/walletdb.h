...
public:
    ...
    bool WriteZSTREAMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSTREAMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSTREAMSeed_deprecated(uint256& seed);
    bool EraseZSTREAMSeed();
    bool EraseZSTREAMSeed_deprecated();

    bool WriteZSTREAMCount(const uint32_t& nCount);
    bool ReadZSTREAMCount(uint32_t& nCount);
